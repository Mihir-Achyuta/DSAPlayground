  ...
  getUserData()
    ...
    .then((doc) => {
      ...
      if (treeFound) {
        ...
        const elementFound = binarySearchTree.search(
          parseInt(req.params.number)
        );

        res.json({
          message: `Binary Search Tree with name ${req.params.name} ${
            !elementFound ? "doesn't contain" : "contains"
          } element ${req.params.number}`,
          error: false,
          code: 200,
          results: treeFound,
        });
